...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    if (verifiedCurrent.length === 1) {
      const clubs = await asAdmin.entities.Club.filter({ club_key: verifiedCurrent[0].club_key });
      if (clubs.length > 0) club = clubs[0];
    } else if (person.current_club_id) {
      club = await asAdmin.entities.Club.get(person.current_club_id);
    }

    if (!club) return Response.json({ status: "sin_club", club: null, fixtures: [], last_results: [], next_fixtures: [], last_sync: null, message: "Actualmente sin club" });
    ...
    if (mappings.length === 0) return Response.json({ status: "no_mapping", club: { id: club.id, club_name: club.club_name, club_key: club.club_key, internal_logo_url: club.internal_logo_url }, fixtures: [], last_results: [], next_fixtures: [], last_sync: null, message: "El club no está vinculado a API-Football" });
    ...
    const fixturesWithRole = clubFixtures.map((f: any) => ({
      ...f,
      is_home: f.home_provider_team_id === providerTeamId,
    ...
    const now = new Date();
    const past = fixturesWithRole.filter((f: any) => new Date(f.fixture_date) < now && f.result)
      .sort((a: any, b: any) => new Date(b.fixture_date).getTime() - new Date(a.fixture_date).getTime())
      .slice(0, 10);
    const upcoming = fixturesWithRole.filter((f: any) => new Date(f.fixture_date) >= now)
      .sort((a: any, b: any) => new Date(a.fixture_date).getTime() - new Date(b.fixture_date).getTime())
      .slice(0, 15);
    ...
    const lastSync = syncLogs.length > 0
      ? [...syncLogs].sort((a: any, b: any) => new Date(b.sync_date).getTime() - new Date(a.sync_date).getTime())[0]
      : null;

    return Response.json({
      status: "ok",
      club: { id: club.id, club_name: club.club_name, club_key: club.club_key, internal_logo_url: club.internal_logo_url },
      provider_mapping: { provider_team_id: mapping.provider_team_id, provider_team_name: mapping.provider_team_name, mapping_status: mapping.mapping_status, last_sync_at: mapping.last_sync_at },
      next_match: nextMatch,
      last_results: past,
      next_fixtures: upcoming,
      all_fixtures: fixturesWithRole,
      last_sync: lastSync ? { sync_date: lastSync.sync_date, queries_consumed: lastSync.queries_consumed, queries_remaining: lastSync.queries_remaining, fixtures_imported: lastSync.fixtures_imported, fixtures_updated: lastSync.fixtures_updated } : null
    });
  } catch (error: any) {
