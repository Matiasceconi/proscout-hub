  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <LineChart data={avgData}>
        ...
        <Tooltip contentStyle={{ borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 12 }} formatter={(v, n) => n === 'avg' ? [v?.toFixed(2), 'Prom. móvil 5p'] : [v, 'Rating']} />
        <Line type="monotone" dataKey="rating" stroke="#3b82f6" strokeWidth={1} dot={{ r: 3, fill: (d) => d?.starter ? '#22c55e' : '#f59e0b' }} />
        <Line type="monotone" dataKey="avg" stroke="#0f172a" strokeWidth={2} strokeDasharray="5 5" dot={false} />
      </LineChart>
    </ResponsiveContainer>
  );
}
    ...
    <ResponsiveContainer width="100%" height={180}>
      <BarChart data={chartData}>
        ...
        <Bar dataKey="minutos" radius={[4, 4, 0, 0]}>
          {chartData.map((d, i) => <Cell key={i} fill={d.starter ? '#22c55e' : '#f59e0b'} />)}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
...
function ProductionChart({ data }) {
  ...
  const chartData = data.map(m => {
    ...
    return { date: formatDate(m.fixture_date).slice(0, 6), acumulado: cumulative };
  });
  return (
    <ResponsiveContainer width="100%" height={180}>
      <LineChart data={chartData}>
        ...
        <Tooltip contentStyle={{ borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 12 }} />
        <Line type="monotone" dataKey="acumulado" stroke="#22c55e" strokeWidth={2} dot={{ r: 3 }} />
      </LineChart>
    </ResponsiveContainer>
  );
}
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <RadarChart data={data}>
        ...
        <Radar dataKey="value" stroke="#0f172a" fill="#0f172a" fillOpacity={0.15} strokeWidth={2} />
      </RadarChart>
    </ResponsiveContainer>
  );
}

export default function StatsCharts({ matchStats, seasonStats, position }) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <div className="border border-slate-200 rounded-lg p-4">
        ...
        <div className="flex gap-3 mt-2 text-xs">
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500" /> Titular</span>
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-500" /> Suplente</span>
        </div>
      </div>
      <div className="border border-slate-200 rounded-lg p-4">
        <h3 className="text-sm font-semibold text-slate-700 mb-2">Participación (minutos)</h3>
        <MinutesChart data={sorted} />
      </div>
      <div className="border border-slate-200 rounded-lg p-4">
