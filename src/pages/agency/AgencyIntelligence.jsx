...
const tabs = [['priorities','Prioridades'],['tasks','Seguimientos'],['services','Servicios y evolución']];
  ...
  return <div className="mx-auto max-w-[1440px] p-4 md:p-8 space-y-6">
    ...
    {tab==='priorities'&&<>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">{filtered.map(p=><article key={p.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 pt-4 border-t flex flex-wrap gap-2">{data.permissions.calendar&&<Button size="sm" variant="outline" onClick={()=>openTask(p)}><Plus className="w-3.5 h-3.5 mr-1"/>Seguimiento</Button>}<Link className="inline-flex items-center gap-1 text-xs text-emerald-700 ml-auto" to={'/agency/players/'+p.id}>Abrir ficha<ArrowUpRight className="w-3.5 h-3.5"/></Link></div>
      </article>)}</div>
